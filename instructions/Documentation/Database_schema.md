...
| -------------------- | ------------ | ------------------------------------ | ------------------------ |
| job_id               | INT (PK)     | AUTO_INCREMENT                       | Unique job identifier    |
| employer_id          | INT          | NOT NULL, FK → employers.employer_id | Company posting job      |
...
| eligible_branches    | TEXT         | NULL                                 | Allowed departments      |
| max_backlogs         | INT          | NULL                                 | Maximum allowed backlogs |
| passing_year         | INT          | NULL                                 | Required passing year    |
| application_deadline | DATE         | NOT NULL                             | Last date to apply       |
| placement_drive_date | DATE         | NOT NULL                             | Placement drive date     |
| created_at           | TIMESTAMP    | NOT NULL                             | Job posting time         |

### Notes

* A job **must always belong to an employer**.
* Critical fields like `job_title`, `salary_package`, `application_deadline`, and `placement_drive_date` are mandatory.
* Eligibility-related fields are optional but must be handled carefully in backend logic.
